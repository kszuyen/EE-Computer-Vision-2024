...
def train(
    ...
    best_acc = max(val_acc_list) if len(val_acc_list) != 0 else 0.0
    ...
    for epoch in range(last_epoch + 1, cfg.epochs):
        ...
        for batch, data in enumerate(train_loader):
            sys.stdout.write(
                f"\r[{epoch + 1}/{cfg.epochs}] Train batch: {batch + 1} / {len(train_loader)}, Loss: {loss:.4f}"
            )
            sys.stdout.flush()
            # Data loading.
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(val_loader):
                sys.stdout.write(
                    f"\r[{epoch + 1}/{cfg.epochs}] Val batch: {batch + 1} / {len(val_loader)}, Loss: {loss:.4f}"
                )
                sys.stdout.flush()
...
def main():
    ...
    exp_name = cfg.model_type + "_" + cfg.exp_name
    ...
    checkpoint_exists = os.path.exists(os.path.join(model_save_dir, "checkpoint.pth"))
    if checkpoint_exists:
        print("Loading checkpoint...")
        checkpoint = torch.load(os.path.join(model_save_dir, "checkpoint.pth"), map_location=device)
