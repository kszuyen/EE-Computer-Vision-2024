...
def warping(src, dst, H, direction="b"):
    ...
    # backward warping: inverse transform destination pixels to source pixels
    if direction == "b":
        # TODO: 1.meshgrid the (x,y) coordinate pairs (with destination points)
        ...
        des_coor = np.concatenate((xx, yy, ones), axis=1).astype(np.int32)

        # TODO: 3.apply H_inv to the destination pixels and retrieve (u,v) pixels, then reshape to (ymax-ymin), (xmax-xmin)
        Resource_pixel = H_inv.dot(des_coor.T).T  # (N * 3)

        # divide the first two columns by the third, converts the homogeneous coordinates back to Cartesian coordinates
        ...
        # TODO: 2.reshape the resource pixels as N x 3 homogeneous coordinate
        xx, yy = xx.flatten()[:, np.newaxis], yy.flatten()[:, np.newaxis]
        ones = np.ones((len(xx), 1))
